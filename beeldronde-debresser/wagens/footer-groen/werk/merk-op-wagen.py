"""KOPIE (25-09-2026) van beeldronde-debresser/wagens/werk/merk-op-wagen.py voor de footerwagen in het groen
(footer-wagen-groen, versie 2). Het origineel blijft onaangeroerd. Verschillen: eigen vlakken voor versie 2 van
die ronde (6336 x 2688), de logo's worden uit de map van het origineel gelezen, de groene band en de witbasis
worden alleen binnen de laadbak gemeten (achter deze wagen staat een bos, dat anders als 'groen' meetelt) en
de uitvoer mag een .jpg zijn.
...
De vlakken worden met een homografie op de laadbak gezet en krijgen de schaduw van de wagen mee.
    ...
    python merk-op-wagen.py <map met versie-<nr>.jpg> <nr> [uitvoerbestand]
"""
import sys
...
LOGOS = Path(__file__).resolve().parents[2] / "werk" / "logos"
...
# Per versie: de vier hoeken van de laadbakzijde (voor-boven, achter-boven, achter-onder, voor-onder)
# en de losse vlakken op de cabine: (x, y, breedte, hoogte, helling) in beeldpixels.
# Footerwagen versie 2, gemeten op het ruwe beeld (rooster per 25 px): laadbakhoeken op de hoekstijlen, verdwijnpunt
# van de zijkant ongeveer (9464, 1668), dus op de cabine loopt een horizontale lijn 0,055-0,06 omlaag naar rechts.
# Deur: onder de spiegelarm (tot y 1377) en boven de sierlijn en de deurgreep (vanaf y 1477). Het beeldmerk staat
# op het lage paneel achter de deur (x 4037-4180), onder de naad op y 1334: direct onder het groene vlak is maar
# 60 px ruimte. As van het front: Renault-logo 3415-3454, bovenste sleuf 3313-3561, grille 3327-3552,
# kenteken 3372-3497, dus x 3437.
VERSIES = {
    2: {"bak": [(4206, 785), (5343, 976), (5358, 1693), (4222, 1700)],
        "deur": (3818, 1395, 136, None, 0.055),
        "beeldmerk": (4082, 1350, 52, None, 0.06),
        "frontvlak": [(3240, 1420), (3770, 1400), (3770, 1545), (3240, 1555)], "as": 3437},
}

W, H = 2700, 1000   # de laadbakzijde plat, lengte : hoogte = 2,7 : 1
...
def bak_canvas():
    ...
    plaats(c, erkende(), 100, 85, 420, 205)                                   # A, Erkende Verhuizers
    plaats(c, group_of_companies(60), 100, 265, 560, 370)                     # A, De Bresser Group of Companies
    plaats(c, bijsnijden(Image.open(LOGOS / "2024-03_De-Bresser-Top-movers.png").convert("RGBA")),
           680, 105, 1060, 285)                                              # B, Top Movers
    plaats(c, logo_zonder_kroon(), 2110, 90, 2620, 670, "midden")            # D, groot logo zonder kroon
    for (naam, nummer), cx in zip(VESTIGINGEN, (1030, 1620, 2190)):         # E, vestigingen
        v = vestiging(naam, nummer, 34)
        c.alpha_composite(v, (int(cx - v.width / 2), 800))
    return c


def slogan_canvas(band):
...
def main(map_, nr, doelpad=None):
    cfg = VERSIES[nr]
    beeld = Image.open(Path(map_) / f"versie-{nr}.jpg").convert("RGB")
    grootte = beeld.size
    rect = [(0, 0), (W, 0), (W, H), (0, H)]

    # waar ligt de gegenereerde groene band op het canvas?
    a = np.asarray(beeld).astype(int)
    groen = (a[:, :, 1] > a[:, :, 0] + 50) & (a[:, :, 1] > a[:, :, 2] + 50)
    bakmasker = Image.new("L", grootte, 0)
    ImageDraw.Draw(bakmasker).polygon(cfg["bak"], fill=255)
    inbak = np.asarray(bakmasker) > 0
    groen = groen & inbak
    naar_canvas = homografie(rect, cfg["bak"])    # beeldpunt -> canvaspunt
    ys, xs = np.nonzero(groen)
    pts = np.array([punt(naar_canvas, x, y) for x, y in zip(xs[::7], ys[::7])])
    binnen = (pts[:, 0] > 0) & (pts[:, 0] < W) & (pts[:, 1] > 0) & (pts[:, 1] < H)
    pts = pts[binnen]
    # het massieve deel van de band: de rijen waar de meeste groene punten liggen
    y0, y1 = np.percentile(pts[:, 1], 8), np.percentile(pts[:, 1], 92)
    massief = pts[(pts[:, 1] > y0) & (pts[:, 1] < y1)]
    kol = np.histogram(massief[:, 0], bins=54, range=(0, W))[0]
    vol = np.nonzero(kol > kol.max() * 0.6)[0]
    x0 = vol.min() * W / 54
    x1 = vol[np.nonzero(np.diff(vol) > 1)[0][0]] * W / 54 + W / 54 if np.any(np.diff(vol) > 1) else vol.max() * W / 54
    band = (x0, y0, x1, y1)
    print(f"versie {nr}: band op canvas x {x0:.0f}-{x1:.0f}, y {y0:.0f}-{y1:.0f}")

    wit = np.asarray(beeld.convert("L")).astype(float)
    witbasis = np.percentile(wit[inbak & ~groen & (wit > 170)], 75)
    bandbasis = np.median(np.asarray(beeld.convert("L")).astype(float)[groen])

    laag = warp(bak_canvas(), cfg["bak"], grootte).filter(ImageFilter.GaussianBlur(0.45))
    uit = samenvoegen(beeld, laag, witbasis)
    laag = warp(slogan_canvas(band), cfg["bak"], grootte).filter(ImageFilter.GaussianBlur(0.45))
    uit = samenvoegen(uit, laag, bandbasis)

    cab = Image.new("RGBA", grootte, (0, 0, 0, 0))
    for vlak, sleutel in ((woordmerk_met_regels(), "deur"), (beeldmerk(), "beeldmerk")):
        v, hoeken = cabinevlak(vlak, *cfg[sleutel])
        cab.alpha_composite(warp(v, hoeken, grootte))
    cab.alpha_composite(front_op_as(cfg, grootte))
    uit = samenvoegen(uit, cab.filter(ImageFilter.GaussianBlur(0.6)), witbasis)

    doel = Path(doelpad) if doelpad else Path(map_) / f"versie-{nr}-merk.png"
    uit.save(doel, quality=95, subsampling=0) if doel.suffix.lower() in (".jpg", ".jpeg") else uit.save(doel)
    print(doel)
