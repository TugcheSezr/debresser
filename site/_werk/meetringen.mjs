...
const port=9400+Math.floor(Math.random()*200); const udd=fs.mkdtempSync(path.join(os.tmpdir(),'hs-'));
...
for(const w of [1920,1600,1440,1200,1024,901]){
  ...
  await ev(`document.querySelectorAll('[data-reveal]').forEach(e=>e.classList.add('in'));1`); await sleep(350);
  console.log(w+'px', await ev(`(()=>{const r=s=>{const e=document.querySelector(s);if(!e)return null;const b=e.getBoundingClientRect();return [Math.round(b.left),Math.round(b.top),Math.round(b.width),Math.round(b.height)]};
   const sec=document.querySelector('.ringen').getBoundingClientRect();
   return JSON.stringify({duo:r('.ringen__duo'),fig:r('.ringen__fig'),tekst:r('.ringen__tekst'),cirkel:r('.ringen__cirkel'),zegel:r('.ringen__zegel'),sectieBreedte:Math.round(sec.width)})})()`));
}
